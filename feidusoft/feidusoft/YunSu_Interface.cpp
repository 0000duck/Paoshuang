...
//========================= CYunSu_Interface接口类(作者只要关注这个类) =============================
CYunSu_Interface::CYunSu_Interface()
{
	// 修改默认系统连接数
...
void CYunSu_Interface::YunSu_SoftInit(LPCTSTR lpSoftwareID, LPCTSTR lpSoftwareKEY, LPCTSTR lpQuestionType)
{
...
void CYunSu_Interface::YunSu_UserInit(LPCTSTR lpAccount, LPCTSTR lpPassword)
{
	CGlobalMgr::m_Account = CUtils::WStringToString(lpAccount);
	CGlobalMgr::m_Password = CUtils::WStringToString(lpPassword);
}

BOOL CYunSu_Interface::YunSu_UploadPicByByte( LPBYTE lpBytePic,  DWORD dwPicSize,  string& szResult)
{
	return AnswerUploadPic("image", lpBytePic, dwPicSize, szResult);
}

BOOL CYunSu_Interface::YunSu_UploadPicByLocal(LPCTSTR lpLocalPicPathName,  string& szResult)
{
	
	return AnswerUploadPic("image", lpLocalPicPathName, szResult);
}

BOOL CYunSu_Interface::YunSu_UploadPicByURL(LPCTSTR lpURLPic,  string& szResult)
{
	return SubmitPicURL(CUtils::WStringToString(lpURLPic), szResult);
}

BOOL CYunSu_Interface::YunSu_SubmitErrorID(LPCTSTR lpErrorID,  string& szResult)
{
	/*
	http://api.ysdm.net/reporterror.json
	{"username"}
	{"password"}
	{"id"}
	*/
	...
	CGlobalMgr::PostToYunSu("reporterror.xml", postData, rspMsg, successful);
...
BOOL CYunSu_Interface::YunSu_Register( LPCTSTR lpAccount,  LPCTSTR lpPassword,  LPCTSTR lpEmail,  string& szResult)
{
	/*
	http://api.ysdm.net/register.xml
	{"username"}
	{"password"}
	{"email"}
	*/
	...
	string rspMsg = "";
	bool successful = false;
	CGlobalMgr::PostToYunSu("register.xml", postData, rspMsg, successful);
...
BOOL CYunSu_Interface::YunSu_Queryacct( LPCTSTR lpAccount,  LPCTSTR lpPassword,  string& szResult)
{
	/*
	http://api.ysdm.net/info.xml
	{"username"}
	{"password"}
	*/
	...
	string response = "";
	CUtils::PostRequest("api.ysdm.net", "info.xml", postData, response);
...
BOOL CYunSu_Interface::YunSu_Recharge( LPCTSTR lpAccount,  LPCTSTR lpCardID,  LPCTSTR lpCardPassword,  string& szResult)
{
	/*
	http://api.ysdm.net/recharge.xml
	{"username"}
	{"id"}
	{"password"}
	*/
	...
	string rspMsg = "";
	bool successful = false;
	CGlobalMgr::PostToYunSu("recharge.xml", postData, rspMsg, successful);
...
BOOL CYunSu_Interface::AnswerUploadPic(const string& fileName, const LPBYTE lpPicByte, DWORD dwPicSize,  string& szResult)
{
	if (lpPicByte == NULL)
	{
	...
	//设置服务器参数
	MySend.SetServerParam("api.ysdm.net","create.xml", INTERNET_DEFAULT_HTTP_PORT);
...
BOOL CYunSu_Interface::AnswerUploadPic(const string& fileName, const wstring& imagePath,  string& szResult)
{
	/*
	http://api.ysdm.net/create.xml
	{"username"}
	{"password"}
	{"typeid"}
	{"timeout"}
...
BOOL CYunSu_Interface::SubmitPicURL(const string& imageURL,  string& szResult)
{
	/*
	http://api.ysdm.net/create.xml
	{"username"}
	...
	*/
	...
	string response;
	CUtils::PostRequest("api.ysdm.net", "create.xml", postData, response);

	return ProcAnswerResult(response, szResult);
}

BOOL CYunSu_Interface::ProcAnswerResult(const string& response,  string& szResult)
{
	string shownStr = "";
...
{ 
	...
	DWORD dwRet = 0;
	HINTERNET hInternet = InternetOpen(CUtils::StringToWString("YunSuCPP").c_str(),INTERNET_OPEN_TYPE_PRECONFIG,NULL, NULL,NULL);   
	//测试句柄
	if(hInternet < 0 )
	{
		return false;
...
void CGlobalMgr::PostToYunSu(const string& targetPage, const string& postData, string& rspMsg, bool& successful)
{
	string response;
	CUtils::PostRequest("api.ysdm.net", targetPage, postData, response);
	...
	)
{
	...
	//开启连接
	hInternet = InternetOpen((LPCWSTR)"YunSuCPP",INTERNET_OPEN_TYPE_PRECONFIG,NULL,NULL,NULL);   
	if(hInternet < 0 )
	{
		return 101;
	}
	...
	int	SendCode, int RecvCode, BOOL bIsByte, LPBYTE lpPicByte, DWORD dwPicByteSize)
{
	...
	HINTERNET hRequest;
	//连接网络
	hInternet = InternetOpen(CUtils::StringToWString("YunSuCPP").c_str(),INTERNET_OPEN_TYPE_PRECONFIG,NULL,NULL,NULL);   
	if(hInternet < 0 )
	{
		return 101;
	}
