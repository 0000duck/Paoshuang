// YunSu_Interface.h: interface for the CYunSu_Interface class.
//
//////////////////////////////////////////////////////////////////////

//#if !defined(AFX_YunSu_INTERFACE_H__45EF8663_0A4D_4187_AF01_DFC790A5BF5C__INCLUDED_)
//#define AFX_YunSu_INTERFACE_H__45EF8663_0A4D_4187_AF01_DFC790A5BF5C__INCLUDED_
//
//#if _MSC_VER > 1000
//#pragma once
//#endif // _MSC_VER > 1000
...
public:
	...
	void YunSu_SoftInit(LPCTSTR lpSoftwareID,LPCTSTR lpSoftwareKEY,LPCTSTR lpQuestionType);
	// 用户设置, 这个函数必须要调用
	// 参数: lpAccount:用户名; lpPassword:密码;
	void YunSu_UserInit(LPCTSTR lpAccount,LPCTSTR lpPassword);

	// 上传图片字节数组
	// 参数: lpBytePic--图片字节数组; dwPicSize--字节数组长度; szResult--返回结果
	// 返回: TRUE则szResult=正确信息, 否则szResult=错误信息.
	BOOL YunSu_UploadPicByByte(LPBYTE lpBytePic,DWORD dwPicSize,  string& szResult);
	// 上传本地图片
	// 参数: lpLocalPicPathName:本地验证码图片全路径; szResult:返回结果;
	// 返回: TRUE则szResult=正确信息, 否则szResult=错误信息.
	BOOL YunSu_UploadPicByLocal( LPCTSTR lpLocalPicPathName,  string& szResult);
	// 上传URL图片
	// 参数: lpURLPic:网络验证码图片的URL链接; szResult:返回结果;
	// 返回: TRUE则szResult=正确信息, 否则szResult=错误信息.
	BOOL YunSu_UploadPicByURL( LPCTSTR lpURLPic,  string& szResult);
	// 提交错误结果ID
	// 参数: lpErrorID:结果ID;
	// 返回: 返回结果信息.
	BOOL YunSu_SubmitErrorID( LPCTSTR lpErrorID,  string& szResult);

	// 注册
	// 参数: lpAccount:注册用户名; lpPassword:注册密码; lpEmail:注册邮箱; szResult:返回结果;
	BOOL YunSu_Register( LPCTSTR lpAccount,  LPCTSTR lpPassword,  LPCTSTR lpEmail,  string& szResult);
	// 查询
	// 参数: lpAccount:用户名; lpPassword:密码; szResult:返回结果;
	BOOL YunSu_Queryacct( LPCTSTR lpAccount,  LPCTSTR lpPassword,  string& szResult);
	// 充值
	// 参数: lpAccount:用户名; lpCardID:充值卡号; lpCardPassword:充值密码; szResult:返回结果;
	BOOL YunSu_Recharge( LPCTSTR lpAccount,  LPCTSTR lpCardID,  LPCTSTR lpCardPassword,  string& szResult);
...
public:
	...
	static void PostToYunSu(const string& targetPage, const string& postData, string& rspMsg, bool& successful);
...
//#endif // !defined(AFX_YunSu_INTERFACE_H__45EF8663_0A4D_4187_AF01_DFC790A5BF5C__INCLUDED_)
